 ...
 * @param <T>
 */
public class RAMIndexer4<T extends Model> extends AbstractIndexer<T> {
	/**
	 * 分析器
	 */
	...
	private static final Logger logger = Logger.getLogger(RAMIndexer4.class);
	...
	public void init() throws IOException {
		...
		try {
			...
			segmentReader = new IFilterDirectoryReader(dr,
					this.deleteQueue);
		    segmentWriter.deleteDocuments(new Term("simpleId","-1"));
		    logger.info("segmentReader.hasDeletions()"+dr.hasDeletions());
			...
			try {
				if (this.readOnly) {
					...
					if (modelQueue != null) {
						...
						syncBuildDocumentWithoutCommit(content);
					}
				}
	...
	public void judgeRam() {
		logger.debug(">>>>> ramIndexer4.judgeRam rambytesUsed="+this.directory.ramBytesUsed());
	 ...
	 * @throws IOException
	 */
	public void syncBuildDocumentWithoutCommit(T content) {
		// 先判断是否是只读
		if (this.readOnly == false) {
			try {
				logger.debug(">>>> sync begin to consume content(" + content
						+ ")...");
				int num = indexNum.incrementAndGet();
				if (this.readOnly != false) {
					modelQueue.add(content);
					return;
				}
				logger.debug(">>> indexNum increment = " + num);
				...
				logger.error(
						">>>RAMIndexer4.syncBuildDocumentWithoutCommit IOException ",
						e);
			} finally {
				if (indexNum.get() == 1) {
					logger.info(">>>> indexNum ==0 indexCommit");
					indexCommit();
				}
				indexNum.decrementAndGet();
			}
		} else {
			modelQueue.add(content);
			logger.debug("RAMIndexer4.syncBuildDocumentWithoutCommit merging >> ");
		}
	}
	
	public void indexCommit() {
		try {
			logger.debug(">>>>>RAMIndexer4 committing");
			if (this.iwriter.isOpen() && readOnly == false) {
				this.iwriter.commit();
			}
		} catch (IOException e) {
			logger.error("RAMIndexer4.indexCommit IOException ", e);
		}
	}
	...
	private synchronized void merge2SingleFSDirectory() throws IOException,
		...
		logger.debug(">>>> merge2SingleFSDirectory start");
		logger.info(">>>> clean delete quene...");
		deleteSegmentIndexes(true);
		logger.info(">>>> begin to merge RAM directory to FS directory("
				+ segmentPath + ")");
		if (this.iwriter.isOpen()) {
			logger.info(">>>############## iwriter is open");
			while (indexNum.get() != 0) {
				logger.info(">>>> merge2SingleFSDirectory waiting ...."
						+ segmentPath + ")");
				Thread.sleep(10);
			}
			this.iwriter.close();
		} else {
			logger.info(">>>############## iwriter is close");
		}
		...
		//if (segmentReader == null) {
			DirectoryReader dr = DirectoryReader.open(segmentDirectory);
			segmentReader = new IFilterDirectoryReader(dr, this.deleteQueue);
		//}
	}
	...
	public synchronized void deleteSegmentIndexes(boolean clean)
		...
		logger.debug(">>>> delete " + i + " terms from segment directory");
		segmentWriter.commit();
	}

	public void deleteSegmentIndexesTest(String id){
		try {
			Term term = new Term("simpleId", id);
			segmentWriter.deleteDocuments(term);
			segmentWriter.commit();
		} catch (IOException e) {
			logger.error("RAMIndexer4.deleteSegmentIndexesTest IOException ", e);
		}
	}
	...
	public DirectoryReader getReader() {
		try {
			if (reader == null) {
				reader = DirectoryReader.open(iwriter,true);
			} else {
				DirectoryReader dr = DirectoryReader.open(iwriter,true);
				if (dr != null) {
					// don't close this reader because it maybe handled by other
					// point-in-time threads which try to use it to read.
					// reader.close();
	...
	public DirectoryReader getSegmentReader() {
		try {
			if (segmentReader != null) {
				DirectoryReader dr = segmentReader;
				segmentReader = IFilterDirectoryReader.doWrapDirectoryReader(
						segmentReader, deleteQueue);
				if (dr != null) {
					// don't close this reader because it maybe handled by other
					// point-in-time threads which try to use it to read.
					//segmentReader.close();
